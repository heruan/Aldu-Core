...
class Response extends Net\HTTP
{
  ...
  protected function setRange($range, $filesize, &$first, &$last)
  {
    ...
    if ($first > $last) {
      // Unsatisfiable range
      $this->status(416);
      $this->header('Content-Range', "*/$filesize");
      return false;
    }
    return true;
  }

  protected function serveRange($range)
  {
    $ranges = array();
    ...
    $total = strlen($this->_body);
    if (count($ranges)) {
      $this->status(206);
      $this->header('Accept-Ranges', $accept);
      if (count($ranges) > 1) {
        $content_length = 0;
        ...
        $this->type("multipart/x-byteranges; boundary=$boundary");

        $body = '';
        foreach ($ranges as $range) {
          if (!$this->setRange($range, $length, $first, $last)) {
            return false;
          }
          $body .= "\r\n--$boundary\r\n";
          $body .= "Content-Type: {$this->_contentType}\r\n";
          $body .= "Content-Range: $accept $first-$last/$length\r\n\r\n";
          $body .= substr($this->_body, $first, $last - $first + 1);
        }
        $body .= "\r\n--$boundary--\r\n";
        $this->_body = $body;
      }
        ...
        $this->header("Content-Length", ($last - $first + 1));
        $this->header("Content-Range", "$accept $first-$last/$total");
        $this->_body = substr($this->_body, $first, $last - $first + 1);
      }
    }
    return true;
  }
  ...
  {
    switch ($this->request->sapi) {
    case 'cli':
      break;
    default:
      $this->header('X-Aldu-Response-Messages', json_encode($this->_messages));
      if (isset($this->_headers['Location']) && $this->_status === 200) {
        $this->status(302);
      }

      if ($range = $this->request->header('Range')) {
        $this->serveRange($range);
      }
   ...
   * Stack a message.
   *
   * @param string $message the string message to stack
   * @return void
   */

  public function message($message, $priority = LOG_INFO)
  {
    if (!isset($this->_messages[$priority])) {
      $this->_messages[$priority] = array();
    }
    $this->_messages[$priority][] = $message;
  }

  public function error($message)
  {
    $this->message($message, LOG_ERR);
  }

  public function debug()
  {
    foreach (func_get_args() as $data) {
      $this->message(htmlentities(var_export($data, true)), LOG_DEBUG);
    }
  }
  ...
  public function body($content = null)
  {
    ...
    return $this->_body = (string) $content;
  }
  ...
  /**
   * Queries and sets valid HTTP response codes and messages.
   *
